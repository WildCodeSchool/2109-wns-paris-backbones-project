	...
	async addRole(@Arg("createRoleInput") input: CreateRoleInput) {
		try {
			...
			if (!role.title) {
				...
				errorHandler(`Role with title ${role.title} already exists on this project`)
	...
	async updateRole(
		...
		try {
			...
			if (usersNotOnProject) {
				errorHandler(`User with id ${usersNotOnProject[0].id} is not referenced on the project ${role.project?.id}`)
			} else if (findSameTitle(await roles, input.title, roleId)) {
				errorHandler(`Role with title ${input.title} already exists on this project`)
			}
