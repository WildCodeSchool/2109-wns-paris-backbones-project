...
export class ProjectResolver {
    ...
    // CREATE
    @Mutation(() => Project)
    async addProject(@Arg("CreateProjectInput") CreateProjectInput: CreateProjectInput) {
        let newProjectId = 0;
        try {
    ...
    async updateProject(
        ...
        try {
            await Project.update(projectId, CreateProjectInput).then((result) => {
                if (result) {
                    console.log("Succesfully update: ", result);
                } else {
                    console.log("ERROR: We can't update this Project", result);
                }
            });
        } catch (error) {
            console.log(error);
