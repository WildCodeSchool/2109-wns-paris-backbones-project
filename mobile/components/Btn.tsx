...
export const Btn = ({ content, buttonType, onPress }: IBtn) => {
  ...
  const gradient = buttonType === 'enabled' ? `primary-linear` :
    buttonType === 'disabled' ? `light-linear` : `secondary-linear`;
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
    >
      <GradientWrapper gradientName={gradient} style={tw`w-32 h-12 rounded-xl flex-row justify-center items-center`}>
        <Text style={tw.style({
